...
Большая часть комбо рассчитана на игру с другом: стойте рядом, держите пустую основную руку и следите за подсказками на HUD. Чем лучше тайминг, скорость и синхронность, тем мощнее результат.
...
| **Обычный Dap** | Оба заряжают `G` / `Shift + G` и отпускают рядом лицом друг к другу | Dap с тиром от Weak до Fire; результат зависит от заряда, скорости и огня |
| **Perfect Dap** | Оба полностью заряжены и отпускают `G` почти синхронно | Усиленный dap, шоквейв, impact-кадр и шанс перейти в продвинутое комбо |
| **Great Dap → QTE** | Получить сильный dap и нажимать `G` / `H` по подсказкам | Запускается QTE-цепочка, которая усиливает эффект при успешных нажатиях |
...
Полная шпаргалка с условиями, таймингами и причинами, почему комбо может не сработать: [COMBOS.md](COMBOS.md).
